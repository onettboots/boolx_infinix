...
VER="Nethunter"
KERNEL_DIR=`pwd`
REPACK_DIR=$HOME/AnyKernel3GKI
ZIP_MOVE=$HOME/Boolx
BASE_AK_VER="Bool-X-GKI-"
DATE=`date +"%Y%m%d-%H%M"`
...
function create_out {
		echo
		git clone https://github.com/onettboots/boolx_anykernel.git -b GKI $REPACK_DIR && mkdir $ZIP_MOVE
}
function clean_all {
		echo
...
case "$cchoice" in
	...
	x86|X86 )
		...
		wget https://github.com/onettboots/boolx-clang-build/releases/download/Boolx-21/boolx-clang21.tar.gz -P $SAVEHERE
 		cd $SAVEHERE
		echo "Extracting Boolx Clang 21.0.0 to $HOME/toolchains/:"Add commentMore actions
